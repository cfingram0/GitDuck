...
static duckGfx::IModel * loadMeshHelper(const char * filename) {
  Assimp::Importer importer;
  const aiScene * scene = importer.ReadFile("uv_sphere2.blend", aiProcess_Triangulate);
  ...
  duckGfx::IModel * toReturn = duckGfx::CreateMesh(fmt, vertBuffer, numVerts, idxBuffer, idxCount);
  delete[] vertBuffer;
  delete[] idxBuffer;
...
void ModelLoad_Test::Init() {
  ...
  m_model3 = loadMeshHelper("uv_sphere2.blend");
